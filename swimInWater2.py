...
class Solution:
    def swimInWater(self, grid) -> int:
        ...
        hq = [(grid[0][0], 0, 0)]
        while len(hq):
            c, basex, basey = heapq.heappop(hq)
            if visited[basex][basey]:
                continue
            visited[basex][basey] = 1
            for d in directions:
                newx, newy = basex + d[0], basey + d[1]
                if 0 <= newx < n and 0 <= newy < n:
                    if visited[newx][newy]: continue
                    cost[newx][newy] = max(cost[basex][basey], grid[newx][newy])
                    heapq.heappush(hq, (cost[newx][newy], newx, newy))
        return cost[n - 1][n - 1]


if __name__ == '__main__':
    grid = [[375, 396, 190, 333, 304, 65, 99, 262, 214, 344, 167, 328, 124, 207, 253, 173, 55, 243, 132, 163],
